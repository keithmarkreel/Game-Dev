...
func _create_match_make_request(
	...
	# --- THE FIX: We read the flat JSON response directly ---
	# print("DEBUG - SERVER RESPONSE: ", response) # Optional: uncomment to see server data
	
	var room = CRoom.new(response["name"], schema_type)
	room.room_id = response["roomId"]
	room.session_id = response["sessionId"]
	
	# Pass the whole 'response' dictionary into the endpoint builder
